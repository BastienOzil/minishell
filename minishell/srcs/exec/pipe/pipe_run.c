...
/*   Updated: 2025/07/26 10:50:45 by aurelia          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void child_exec(t_cmd *cmd, int in_fd, int pipefd[2], char ***envp)
{
    ...
    // AJOUTEZ CES VÉRIFICATIONS EN PREMIER
    if (!cmd || !cmd->args || !cmd->args[0])
    {
        exit(127);  // Code d'erreur "commande non trouvée"
    }
    
    handle_redirections(cmd);
    if (in_fd != 0)
        redirect_fd(in_fd, STDIN_FILENO);
    if (cmd && cmd->next)  // ← Cette vérification est maintenant redondante
    {
